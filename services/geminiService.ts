...
const ai = new GoogleGenAI({ apiKey: "AIzaSyAzsSzKmxEKA4y1lAYnKXpfJFxmj9HsGbY" });
  ...
  try {
    ...
    if (config.outputFormat === 'VIDEO') {
      ...
      if (downloadLink) {
        ...
        const videoRes = await fetch(`${downloadLink}&key=AIzaSyAzsSzKmxEKA4y1lAYnKXpfJFxmj9HsGbY`);
